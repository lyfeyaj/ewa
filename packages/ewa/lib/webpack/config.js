...
const ChangeFileNamePlugin = require('./plugins/ChangeFileNamePlugin');
...
function makeConfig() {
  ...
  if (IS_DEV && options.autoCleanUnusedFiles) {
    let excludes = null;
    if (PLATFORM !== "baidu") {
      excludes = options.copyFileTypes.map(fileType => {
        return `**/*.${fileType}`;
      }).concat([
      ...
      excludes = ["**/*.js", "**/*.css", "**/*.swan", "**/*.json"].concat([
        // 排除公共库文件 和 对应的 sourceMap 文件
        options.commonModuleName,
        `${options.commonModuleName}.map`,
      ])
